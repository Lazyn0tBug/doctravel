<template>
  <div :class="['overlay', showConfirmation ? 'showElement' : 'hideElement']"></div>
  <div :class="['confirmation', showConfirmation ? 'showElement' : 'hideElement']">
    <i class="fas fa-check-circle confirmation__icon"></i>
    <h1 class="confirmation__heading">Thank you for your order</h1>
    <p class="confirmation__text">You will receive an email confirmation shortly.</p>
    <div class="confirmation__order">
      <div class="confirmation__order__left">
        <div class="product-container" v-if="cart.length > 0">
          <div class="product" v-for="product in orderSelection" :key="product.id">
            <div class="product__left">
              <img :src="editSrc(product)" :alt="product.name" class="product__left__image" />
              <div class="product__left__info">
                <h2 class="product__left__info__name">
                  {{ editedSlug(product) }}
                </h2>
                <p class="product__left__info__price">$ {{ separator(product.price) }}</p>
              </div>
            </div>
      ...
      <div class="confirmation__order__right">
        <div class="confirmation__order__right__text">
          ...
          <p class="confirmation__order__right__text__total">$ {{ separator(total) }}</p>
        </div>
      </div>
    </div>
