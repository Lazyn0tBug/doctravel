  ...
  <main>
    ...
    <section class="overview">
      ...
      <div class="overview__text">
        <p class="overview__text__tag" v-show="currentProduct.new">New product</p>
        ...
        <div class="overview__text__btn-section">
          <div class="overview__text__btn-section__number">
            <button class="overview__text__btn-section__number__less" @click="decreaseTotal">
              -
            </button>
            <p class="overview__text__btn-section__number__value">
              {{ total }}
            </p>
            <button class="overview__text__btn-section__number__more" @click="increaseTotal">
              +
            </button>
          </div>
          <button
    ...
    <section class="details">
      <div class="details__features">
        ...
        <p class="details__features__text" v-for="paragraph in editedText" :key="paragraph">
          {{ paragraph }}
        </p>
      </div>
      <div class="details__box">
        ...
        <ul class="details__box__list">
          <li
            ...
            :key="item.item"
          >
            <p class="details__box__list__item__quantity">{{ item.quantity }}x</p>
    ...
    <section class="gallery">
      ...
      <div class="gallery__right">
        <img :src="gallerySrc('third')" :alt="`${currentProduct.name} presentation image`" />
      </div>
    </section>
...
export default {
    ...
    gallerySrc(index) {
      if (index === 'first') {
        if (this.windowSize === 'mobile') {
          return require(`../${this.currentProduct.gallery.first.mobile.slice(2)}`)
        } else if (this.windowSize === 'tablet') {
          return require(`../${this.currentProduct.gallery.first.tablet.slice(2)}`)
        } else {
          return require(`../${this.currentProduct.gallery.first.desktop.slice(2)}`)
        }
      } else if (index === 'second') {
        if (this.windowSize === 'mobile') {
          return require(`../${this.currentProduct.gallery.second.mobile.slice(2)}`)
        } else if (this.windowSize === 'tablet') {
          return require(`../${this.currentProduct.gallery.second.tablet.slice(2)}`)
        } else {
          return require(`../${this.currentProduct.gallery.second.desktop.slice(2)}`)
        }
      } else {
        if (this.windowSize === 'mobile') {
          return require(`../${this.currentProduct.gallery.third.mobile.slice(2)}`)
        } else if (this.windowSize === 'tablet') {
          return require(`../${this.currentProduct.gallery.third.tablet.slice(2)}`)
        } else {
          return require(`../${this.currentProduct.gallery.third.desktop.slice(2)}`)
        }
      }
    }
  },
  ...
  computed: {
    currentProduct() {
      return this.products.slice().find((product) => product.slug === this.$route.params.product)
    },
    editSrc() {
      if (this.windowSize === 'mobile') {
